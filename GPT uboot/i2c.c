...
static int bl_i2c_write(struct udevice *dev, uint8_t reg, uint8_t val)
{
	int	ret = 0;
...
static int bl_i2c_read(struct udevice *dev, uint8_t reg)
{//i2c_reg_read(LM36923_I2C_ADDR,BL_REG_MODE);
	int	ret = 0;
	uint8_t linebuf;
	ret = dm_i2c_read(dev, reg, &linebuf, 1);//dev
	if (ret){
		return i2c_report_err(ret, I2C_ERR_READ);
	}else{
		printf("bl_i2c_read 0x%x = 0x%x\n",reg,linebuf);
	}

	return linebuf;
}
void LM36923_Set_Current(struct udevice *dev,u16 val)
{
	bl_i2c_write(dev,BL_REG_CODE_L,val&0x07);	
	bl_i2c_write(dev,BL_REG_CODE_H,(u8)(val>>3));	
}

static int do_i2c_bl(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
		int	ret = 0;
		uint8_t linebuf[16];
		u32 val = 20;
		...
		struct udevice *bus;
		struct udevice *dev;
		printf("Setting bus to bl i2c %d\n", I2C_CH_BL);
#ifdef CONFIG_DM_I2C
		//ret = i2c_get_cur_bus_chip(chip, &dev);//
		ret = uclass_get_device_by_seq(UCLASS_I2C, I2C_CH_BL, &bus);//
		if (ret) {
			debug("%s: No bus %d\n", __func__, I2C_CH_BL);
			return ret;
		}
       //0x1budevice
		i2c_get_chip(bus, I2C_BL_ADDR, 1, &dev);
		//if (!ret && alen != -1)
		//	ret = i2c_set_chip_offset_len(dev, alen);
		if (ret)
			return i2c_report_err(ret, I2C_ERR_WRITE);
		//
		ret = dm_i2c_read(dev, 0x2f, linebuf, 1);//
		if (ret){
			return i2c_report_err(ret, I2C_ERR_READ);
		}else{
			printf("bl i2c read 0x2f = 0x%x\n",linebuf[0]);
		}
		
		//ret = dm_i2c_write(dev, addr++, &byte, 1);	//
		bl_i2c_write(dev, BL_REG_RESET, 0x01);
		mdelay(10);
		bl_i2c_write(dev, BL_REG_DEVEICE_ON, 0x01);//

		ret = dm_i2c_read(dev, BL_REG_DEVEICE_ON, linebuf, 1);//de
		if (ret){
			return i2c_report_err(ret, I2C_ERR_READ);
		}else{
			printf("BL_REG_DEVEICE_ON 0x10= 0x%x\r\n",linebuf[0]);
		}
		
		mdelay(10);
		bl_i2c_write(dev, BL_REG_MODE, 0x01);//ret = dm_i2c_write(dev, addr++, &byte, 1);
		mdelay(10);

		temp = bl_i2c_read(dev,BL_REG_MODE);
		printf("\r\nBL:REG_MODE:0x%x",temp);
		temp = bl_i2c_read(dev, 0x13);
		bl_i2c_write(dev, 0x13, temp | (0x01<<4));
		temp = bl_i2c_read(dev, 0x13);
		printf("REG_13 = 0x%x\r\n",temp);
		bl_i2c_write(dev, BL_REG_DEVEICE_ON, 0x03);
		mdelay(10);
		bl_i2c_write(dev, BL_REG_DEVEICE_ON, 0x07);
		mdelay(10);
		bl_i2c_write(dev, BL_REG_DEVEICE_ON, 0x0f);
		mdelay(10);
		//i2c_reg_write(LM36923_I2C_ADDR, BL_REG_MODE, 0x01);
		temp = bl_i2c_read(dev,BL_REG_MODE);
		printf(" BL:REG_MODE:0x%x\r\n",temp);
		printf(" bl ma = %d\r\n",val);
		...
		LM36923_Set_Current(dev,value1);
		mdelay(10);
		...
		printf(" BL:REG_MODE:0x%x\r\n",temp);
			temp = bl_i2c_read(dev, BL_REG_MODE);
		printf("BL_REG_MODE = 0x%x\r\n",temp);
			temp = bl_i2c_read(dev, BL_REG_DEVEICE_ON);
		printf("BL_REG_DEVEICE_ON = 0x%x\r\n",temp);
		temp = bl_i2c_read(dev, BL_REG_CODE_H);
		printf("REG_19 = 0x%x\r\n",temp);
		temp = bl_i2c_read(dev, BL_REG_CODE_L);
		printf("REG_18 = 0x%x\r\n",temp);	
		temp = bl_i2c_read(dev, 0x1f);
		printf("REG_ERR = 0x%x\r\n",temp);
		bl_i2c_write(dev, BL_REG_MODE, 0x01);
	
#else
		//ret = i2c_set_bus_num(I2C_CH_BL);
		printf("NO USER CONFIG_DM_I2C!!!\n");
#endif
		if (ret)
			printf("Failure changing bus number (%d)\n", ret);
		return ret;
...
static int do_i2c_ledon(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	struct gpio_desc gpio;
	int value;
	int ret;
	//gpio_request_by_name(dev, "gpio1", 0, gpio, GPIOD_IS_OUT); // 申请gpio
	ret = gpio_to_device(0xff730000+4,&gpio);
	if(ret != 0){
		printf(" gpio_to_device Error:%d\n",ret);
	}
	
	dm_gpio_set_value(&gpio, 1); // 设置gpio输出电平
	//dm_gpio_set_dir_flags(g&pio, GPIOD_IS_IN); // 设置gpio为输入
	//value = dm_gpio_get_value(&gpio); // 读取gpio电平
}

static int do_i2c_ledoff(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	struct gpio_desc gpio;
	int value;
	int ret;
	...
	dm_gpio_set_value(&gpio, 0); // 设置gpio输出电平
...
#endif
	U_BOOT_CMD_MKENT(bl, 1, 1, do_i2c_bl, "", ""),
	U_BOOT_CMD_MKENT(ledon, 1, 1, do_i2c_ledon, "", ""),
	U_BOOT_CMD_MKENT(ledoff, 1, 1, do_i2c_ledoff, "", ""),
