...
static void lcd_drawchars(ushort x, ushort y, uchar *str, int count)
{
	...
	lcd_color_fg = 0xff;
	lcd_color_bg = 0x00;
	lcd_line_length = 1080;
	//dest = (uchar *)(lcd_base + y * lcd_line_length + x * NBITS(LCD_BPP)/8);
	dest = (uchar *)(0x7dc00000 + y * lcd_line_length + x * NBITS(LCD_BPP)/8);
	printf("fb addr:0x%x\n",(int)dest);
	for (row = 0; row < VIDEO_FONT_HEIGHT; ++row, dest += lcd_line_length) {
		...
		uchar *d = dest;
#endif
		printf("fb addr:0x%x  %d\n",(int)d,VIDEO_FONT_HEIGHT);
		for (i = 0; i < count; ++i) {
			uchar c, bits;
...
void PiontBmp(ushort x, ushort y, int color)//0-1080	 0-1080*3
{
	uchar *BmpFileColorBuffer;
	ushort width = 1080;
	ushort BmpBit = 24;
	BmpFileColorBuffer = (uchar *)(0x7dc00000);
	BmpFileColorBuffer[width * BmpBit / 8 * y + (x * BmpBit / 8) + 0] = ((color >> 0) % 256);
	BmpFileColorBuffer[width * BmpBit / 8 * y + (x * BmpBit / 8) + 1] = ((color >> 8) % 256);
	BmpFileColorBuffer[width * BmpBit / 8 * y + (x * BmpBit / 8) + 2] = ((color >> 16) % 256);

}


void ShowChar(int x, int y, int font, int color, int Bcolor, char a)
{
	int j;
	int xp;
	...
	for (sy = 0; sy < 72; sy++)//列
	{
		...
		for (sx = 0; sx < 5; sx++)//行
		{
			fchar = ascii_char[charindex - 31][sy * 5 + sx];
			for (j = 0; j < 8; j++)
			{
				if ((fchar & 0x80) != 0)
				{
					if (color <= 0xffffff) { PiontBmp(xp, y + sy, color); } //0x00FF
					else { }
				}
				else
				{
					if (Bcolor <= 0xffffff) { PiontBmp(xp, y + sy, Bcolor); }
					else { }
				}
				fchar <<= 1;
				xp++;
...
void LcdDrvGotoXy(u32 x,u32 y)
{

}


void LcdDrvWriteRgb(u32 color)
{
		...
		for(hc = 0; hc < font; hc++)
		{
		    ...
		    for(j = 0; j < 8; j++)
			{
				for(wc = 0; wc < font; wc++)
				{
					...
					if(a & 0x80)
					{
						if(color <= 0xffffff){*dest = color>>16;*(dest+1) = color>>8;*(dest+2) = color>>0;}
						else {dest = (uchar *)(0x7dc00000 + y * lcd_line_length + xp * NBITS(LCD_BPP)/8);}
					}
					else	//mode
					{
						if(mode){	//1²»µþ¼Ó	
							if(Bcolor <= 0xffffff){*dest = Bcolor>>16;*(dest+1) = Bcolor>>8;*(dest+2) = Bcolor>>0;}
							else {dest = (uchar *)(0x7dc00000 + y * lcd_line_length + xp * NBITS(LCD_BPP)/8);}
						}else{							
							{dest = (uchar *)(0x7dc00000 + y * lcd_line_length + xp * NBITS(LCD_BPP)/8);}
						}
					}
				}
				a<<=1;
...
void lcd_clear(void)
{
	...
	//memset((char *)gd->fb_base,
	//	COLOR_MASK(0xa512345a),
	//	1080 * panel_info.vl_row);
	///printf("memset fb:%x line_length:%d row:%d\n",gd->fb_base,lcd_line_length,panel_info.vl_row);
	u32 color = 0xff0000;
	u8 *ppixx = (char *)gd->fb_base;
	u32 i,j;
	for(j=0;j<2280;j++)
	{
		for(i=0;i<1080;i++)
...
struct Interface_funcs {	//放在0x3000000
		void (*led)(uint8_t port);
		void (*printf)(const char*__restrict, ...);
		void (*Delay_us)(uint32_t nTime);
		void (*otp_handle)(void);
		void (*func1)(void);
		void (*func2)(void);
		void (*func3)(void);
		void (*func4)(void);
...
struct Interface_app {	
		struct Interface_funcs *funcs;	
		void (*Load_func)(void);	
};


extern void rk3399_leds(int val);

const struct Interface_funcs Interface_func = {	
		rk3399_leds,	
		printf,
...
#define FUNC_ADDR	0x3000000

struct Interface_app *AppInterface = (struct Interface_app *)FUNC_ADDR;
			...
			char *const argv[])
{
	...
	//AppInterface = (struct Interface_app *)FUNC_ADDR;

	//AppInterface->funcs = &Interface_func;
	//printf("cls clear screen inter app!:0x%x   0x%x\n ",AppInterface->Load_func,AppInterface->funcs);
	//AppInterface->Load_func();
	
	//lcd_clear();

	//lcd_puts("w@@@@@@efhsofuhwfieuwifugiegfigegiwueiugsifuwiefgigidsgifweihfiusifugwiefskifgiugwfigewigskdgfiwugeiwgsilfgiswfgiwegdkisgdgfiwgeif");
	//char c[] = "QsfoioewhofihsofhIUGFISDGFIWGIFSIUDG";
	//lcd_drawchars(250,250,c,10);
	//ShowCharc(500,500,48,0xff,0x00,c[0],1);
            ShowChar(900, 500, 0X01, 0XFF00, 0XFF0000, '1');
            ShowChar(600, 500, 0X01, 0XFF0000, 0XFFFFFF, '2');
            ShowChar(700, 500, 0X01, 0XFF, 0XFFFFFF, '3');
            ShowChar(800, 500, 0X01, 0XFF00, 0XFFFFFF, '4');
	return 0;
}
...
//StorageReadLba(param_addr + i * PARAMETER_OFFSET, buf, read_sec) == 0)
static int do_bmp(cmd_tbl_t *cmdtp, int flag, int argc,
			char *const argv[])
{
	...
	printf("\n bmpshow from sd :addr = 0x%x  size = 0x%x\n",addr,size);
	StorageReadLba(addr-0x2000,BmpFileColorBuffer,size);//512
	return 0;
}

U_BOOT_CMD(
	bmpshow,	3,	1,	do_bmp,/*Readreg  0xf0fffee*/
	"bmpshow addr(sd addr) size",
	""
);



static int do_readreg(cmd_tbl_t *cmdtp, int flag, int argc,
			char *const argv[])
{
	unsigned long addr = 0; 
	//printf("\nRun do_readreg\n");
	addr = simple_strtoul(argv[1], NULL, 16);
	printf("\nRead REG :0x%x return :0x%x\n",addr,readl(addr));
	return 0;
}

U_BOOT_CMD(
	readreg,	3,	1,	do_readreg,/*Readreg  0xf0fffee*/
	"Read reg",
	""
);

static int do_writereg(cmd_tbl_t *cmdtp, int flag, int argc,
			char *const argv[])
{
	unsigned long addr = 0,Value=0; 
	//printf("\nRun do_writereg\n");
	addr = simple_strtoul(argv[1], NULL, 16);
...
U_BOOT_CMD(
	writereg,	3,	1,	do_writereg,/*Readreg  0xf0fffee*/
	"Write reg",
	""
);

extern int mipi_dsi_read(unsigned char dsitype,unsigned short reg,unsigned char *data1,u32 n);

static int do_mipi_dsi_read(cmd_tbl_t *cmdtp, int flag, int argc,
			char *const argv[])
{
	unsigned long dsitype = 0,reg=0,n=0;
	unsigned char vvalbuf[32];

	printf("\nRun do_mipi_dsi_read....\n");

	dsitype = simple_strtoul(argv[1], NULL, 16);
	reg = simple_strtoul(argv[2], NULL, 16);
	n = simple_strtoul(argv[3], NULL, 16);

	printf("\nmipi_dsi_read :0x%x  0x%x 0x%x\n",dsitype,reg,n);
	mipi_dsi_read(dsitype,reg,vvalbuf,n);
	return 0;
}

U_BOOT_CMD(
	mipiread,	4,	1,	do_mipi_dsi_read,	/*Readreg  0xf0fffee*/
	"mipi read  (dsitype,reg,n)",
	""
);
