...
static void *get_display_buffer(int size)
{
	...
	void *buf;
	printf("1get_display_buffer memory_end:0x%x--size:0x%x\n",memory_end, size);
	...
	printf("2get_display_buffer memory_end:0x%x--size:0x%x  buf:0x%x  memory_end:0x%x\n",memory_end, size,buf,memory_end);
	return buf;
}
...
int connector_pclist_parse_dt(struct connector_state *conn_state, const void *blob, int panel)
{
	...
	printf("***********connector_pclist_parse_dt\n");
	DBG("enter\n");
...
static int display_logo(struct display_state *state)
{
	...
	printf("logo->mem：0x%x   logo->offset:0x%x\n", logo->mem,logo->offset);
...
static int load_bmp_logo(struct logo_info *logo, const char *bmp_name)
{
	...
	if (!can_direct_logo(logo->bpp)) {
		...
		printf("get_display_buffer dst:0x%x\n", (int)dst);
		if (!dst)
			return -ENOMEM;
	...
	printf("get_display_buffer logo->mem:0x%x\n", (int)logo->mem);
