					 ...
					 struct dsi_panel_cmds *pcmds)
{
	...
	printf("****%s::dcs_cmd=%x %x %x %x len=%d, cmd_cnt=%d\n",
	       __func__, pcmds->buf[0], pcmds->buf[1], pcmds->buf[2], pcmds->buf[3], pcmds->blen, pcmds->cmd_cnt);
	return 0;
}
...
//	**mipi_dsi:tx_buf:0xbd029639 
	...
	if (!cmds)
		return -EINVAL;
	printf("*************rockchip_dsi_panel_send_cmds\n");
...
static int rockchip_dsi_panel_init(struct display_state *state)
{
	...
	printf("*************rockchip_dsi_panel_init\n");
