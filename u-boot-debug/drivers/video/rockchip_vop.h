...
static inline void vop_mask_write(struct vop *vop, uint32_t offset,
				  ...
				  bool write_mask)
{
	//printf("Vop Write1 : 0x%x  , 0x%x , 0x%x ,0x%x ,  0x%x " ,v,vop->regs,vop->regs + offset,(int)vop->regs,(int)(vop->regs + offset));
	if (!mask)
		return;
	...
	writel(v, vop->regs + offset);
	printf("Vop Write : 0x%x  , 0x%x , 0x%x\n" ,v,vop->regs,vop->regs + offset);//,*vop->regs,*vop->regs + offset
}
