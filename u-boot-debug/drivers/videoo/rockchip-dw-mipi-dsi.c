...
({ \
	...
	printf("dsi addr = 0x%x val = 0x%x",addr,val);\
	(cond) ? 0 : -ETIMEDOUT; \
})
				   ...
				   const struct mipi_dsi_msg *msg)
{
	...
	dw_mipi_dsi_read(dsi,0x06,0x0a,vvalbuf,2);
	udelay(2000);
/*
	dw_mipi_dsi_read(dsi,0x06,0x0c,vvalbuf,12);
	udelay(2000);
	dw_mipi_dsi_read(dsi,0x14,0x0a,vvalbuf,12);
	udelay(2000);
	dw_mipi_dsi_read(dsi,0x14,0x0c,vvalbuf,12);
	udelay(2000);
	dw_mipi_dsi_read(dsi,0x06,0xb4,vvalbuf,3);
	udelay(2000);
	dw_mipi_dsi_read(dsi,0x06,0xda,vvalbuf,3);
	udelay(2000);
*/
	return dw_mipi_dsi_gen_pkt_hdr_write(dsi, val);
}
