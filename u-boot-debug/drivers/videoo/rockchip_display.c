...
static int load_bmp_logo(struct logo_info *logo, const char *bmp_name)
{
	...
	if (!can_direct_logo(logo->bpp)) {
		...
		printf("55get_display_buffer size:%d  pdst:%x",size,pdst);
	} else {
		pdst = get_display_buffer(size);
		printf("get_display_buffer size:%d  pdst:%x",size,pdst);
		dst = pdst;
	}
	...
	if (!can_direct_logo(logo->bpp)) {
		...
		dst_size = 768 * 1024 * logo->bpp >> 3;
		dst = get_display_buffer(dst_size);
		printf("16 get_display_buffer size:%d  768pdst:%x",size,pdst);
		if (!dst)
			return -ENOMEM;
