...
int rk32_mipi_dsi_clk_enable(struct dsi *dsi)
{
	...
	/* pclk_mipi_dsi0 or pclk_mipi_dsi1*/
	if(dsi->dsi_id == 0)
		val = (0x1 << 17) | (0x0 << 1);
	else
...
{
	if (cpu_is_rk3288() || cpu_is_rk3399()) {
...
int rk32_mipi_dsi_read_dcs_packet(void *arg, unsigned char *data1, u32 n)
{
	struct dsi *dsi = arg;
	//DCS READ 
	...
	*/
	...
	printf("rk32_mipi_dsi_read_dcs_packet==0x%x\n", rk32_dsi_get_bits(dsi, GEN_PLD_DATA));
	msleep(100);

	//  rk32_dsi_set_bits(dsi, regs[0], max_rd_pkt_size);
	
	msleep(100);
	printf("_____rk32_mipi_dsi_read_dcs_packet==0x%x\n", rk32_dsi_get_bits(dsi, GEN_PLD_DATA)); 
...
static void rockchip_3399_grf_config(int lcdc_id)
{
	...
	if (dsi_num == 1) {
		if (lcdc_id == 1) {
			val = 0x1 << 16 | 0x1;
			writel_relaxed(val, RK_GRF_VIRT + RK3399_GRF_CON20);
		} else {
			val = 0x1 << 16 | 0x0;
			writel_relaxed(val, RK_GRF_VIRT + RK3399_GRF_CON20);
		}
		...
		val |= 0xf << 20 | 0x0 << 4;
		val |= 0xf << 16 | 0x0;
		writel_relaxed(val, RK_GRF_VIRT + RK3399_GRF_CON22);
	} else {
		if (lcdc_id == 1) {
			val = 0x1 << 16 | 0x1;
			val |= 0x1 << 20 | 0x1 << 4;
		...
		val = 0x1 << 20 | 0x0 << 0x4;
		writel_relaxed(val, RK_GRF_VIRT + RK3399_GRF_CON7);

		val = 0xf << 24 | 0x0 << 8;
		val |= 0xf << 20 | 0x0 << 4;
		val |= 0xf << 16 | 0x0;
		writel_relaxed(val, RK_GRF_VIRT + RK3399_GRF_CON23);

		val = 0x1 << 22 | 0x1 << 6;
		val |= 0x1 << 21 | 0x0 << 5;
		writel_relaxed(val, RK_GRF_VIRT + RK3399_GRF_CON24);
...
int rk_dsi_host_parse_dt(const void *blob, struct dsi *dsi)
{
	...
	if (cpu_is_rk3288()) {
		...
		node = fdt_node_offset_by_compatible(blob, 0, "rockchip,rk3399-dsi");
	}
	if(node<0) {
	...
	do{
		if(fdtdec_get_int(blob, node, "rockchip,prop", -1) != dsi->dsi_id){
			...
			if (cpu_is_rk3288()) {
				...
				node = fdt_node_offset_by_compatible(blob, node, "rockchip,rk3399-dsi");
			}
			if(node<0) {
				printf("mipi dts get node failed, node = %d.\n", node);
				return -1;
	...
	if (cpu_is_rk3288()) {
		...
		dsi->host.membase = (void __iomem *)(unsigned long)mipi_regs[1];
		if (!dsi->host.membase) {
			printf("fdtdec get mipi membase fail!!\n");
		}
...
int rk32_mipi_enable(vidinfo_t *vid)
{
	...
	for(id = 0; id < dsi_number;) {
		...
		printf("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@refresh_mode:%d,real_freq:%d",dsi_screen->refresh_mode,dsi_screen->pixclock);
